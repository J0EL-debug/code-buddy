...
import { GitHubService, FileContentWithContext } from '../github/github.service';

/**
 * Issue Verifier Service
 * Validates LLM-reported issues before posting to GitHub
...
export class IssueVerifier {
  ...
  private async verifyImportIssue(
    ...
    try {
      const fullFile = await this.githubService.getFileContent(
        context.owner,
        context.repo,
        context.filePath,
        context.sha,
  ...
  private async verifyDefinitionIssue(
    ...
    try {
      const extendedContext = await this.githubService.getFileContentWithContext(
        context.owner,
        context.repo,
        context.filePath,
        context.sha,
