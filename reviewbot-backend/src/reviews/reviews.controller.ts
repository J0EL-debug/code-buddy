...
export class ReviewsController {
  ...
  @ApiQuery({
    name: 'source',
    required: false,
    enum: ['all', 'github', 'adhoc'],
    description: 'Which review source to count - GitHub PR reviews, ad-hoc (paste/upload) reviews, or both (default: all)',
  })
  @ApiResponse({ status: 200, description: 'Statistics retrieved successfully' })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  async getStats(
    @Query('timeRange') timeRange?: 'day' | 'week' | 'month' | 'year',
    @Query('source') source?: 'all' | 'github' | 'adhoc',
  ) {
    return this.reviewsService.getStats(timeRange || 'week', source || 'all');
  }
  ...
  @ApiQuery({
    ...
    required: false,
    enum: ['all', 'github', 'adhoc'],
    description: 'Which review source to include (default: all)',
  })
  ...
  async getTimeline(
    ...
    return this.reviewsService.getTimeline({ projectId, developerId, days, source: source || 'all' });
  }
